  ...
  with g.as_default():
    with tf.device('/cpu:0'):
      with tf.name_scope('input'):
        features, labels = mnist_reader.read_trainset_to_ram()
        x_p, y_p, x_batch_tr, y_batch_tr, iter_trainset = mnist_reader.get_batch_use_tfdata(features,labels)
    with tf.name_scope('model'):
      tr_model = LeNet5_CLASSIFY(x_batch_tr, y_batch_tr, LeNet5_CLASSIFY.train)
